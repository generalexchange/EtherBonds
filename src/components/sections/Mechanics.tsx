...
export function Mechanics() {
  return (
    <section
      id="mechanics"
      className="bg-charcoal-950 px-6 py-24 text-parchment-50 lg:px-10"
    >
      <div className="mx-auto max-w-[1200px]">
        ...
        <FadeIn delay={0.16} className="mt-6 max-w-prose text-base text-parchment-50/70">
        ...
        <div className="mt-16 grid grid-cols-1 gap-10 lg:grid-cols-4 lg:gap-0">
          {COPY.mechanics.steps.map((step, i) => (
            <FadeIn
              key={step.n}
              delay={0.08 * (i + 3)}
              className={`relative border-l border-parchment-50/20 pl-6 ${
                i === 0 ? "lg:border-l-0 lg:pl-0" : "lg:pl-8"
              }`}
            >
              <p className="font-serif text-4xl text-parchment-50/30 md:text-5xl">{step.n}</p>
              <p className="mt-4 text-[11px] uppercase tracking-eyebrow text-parchment-50/80">
                {step.title}
              </p>
              <p className="mt-3 max-w-[18rem] text-sm leading-relaxed text-parchment-50/70">
                {step.body}
              </p>
            </FadeIn>
          ))}
        ...
        <FadeIn delay={0.48} className="mt-16 max-w-[720px] text-sm italic text-parchment-50/60">
